# Anthropic İstem Önbelleğe Alma (Prompt Caching)

Anthropic; haiku, sonnet ve opus ailelerinden birçok son teknoloji model sunar.

Bu modellerin LlamaIndex ile nasıl kullanılacağını öğrenmek için okumaya devam edin!
...
#### Belirteç Oluşturucuyu (Tokenizer) Ayarlayın

İlk olarak, TikToken'dan biraz farklı olan belirteç oluşturucuyu ayarlamak istiyoruz. Bu, kütüphane genelinde belirteç sayımının doğru yapılmasını sağlar.

**NOT**: Anthropic yakın zamanda belirteç sayma API'sini güncelledi. Claude-2.1 gibi eski modeller, Anthropic python istemcisinin en yeni sürümlerinde artık belirteç sayımı için desteklenmemektedir.
...
Bir istem (prompt) ile `complete` çağrısı yapabilirsiniz:
...
```
    ...
    Paul Graham bir bilgisayar programcısı, girişimci, risk sermayedar ve denemecidir. İşte tanındığı başlıca noktalar:
    
    **Y Combinator**: 2005 yılında, Airbnb, Dropbox, Stripe ve Reddit gibi şirketlerin kurulmasına yardımcı olan bu oldukça etkili girişim hızlandırıcısını kurucu ortaklarından biri olmuştur. Y Combinator, erken aşamadaki girişimlere çekirdek fon ve mentorluk sağlar.
    
    **Programlama**: Programlama topluluğunda saygın bir isimdir; özellikle Lisp programlama dili üzerindeki çalışmaları ve 1990'larda ilk web tabanlı uygulama olan Viaweb'i (daha sonra Yahoo'ya satılarak Yahoo Store olmuştur) ortaklaşa kurmasıyla tanınır.
    
    **Yazarlık**: Graham, paulgraham.com adresindeki web sitesinde yayınladığı girişimler, teknoloji, programlama ve toplum üzerine düşündürücü denemeleriyle tanınır. Denemeleri teknoloji çevrelerinde geniş çapta okunur ve bir girişimin nasıl kurulacağı, inovasyonun doğası ve sosyal yorumlar gibi konuları kapsar.
    
    **Kitaplar**: "Hackers & Painters" ve "On Lisp" dahil olmak üzere birkaç kitabın yazarıdır.
    
    **Etki**: Hem Y Combinator'ın etkisi hem de girişimcilik ve teknoloji üzerine yazdığı yazılarıyla Silikon Vadisi'nin girişim ekosistemindeki en etkili kişilerden biri olarak kabul edilir.
    
    Graham, analitik düşünce yapısı ve iş dünyası, teknoloji ve kültür üzerindeki aykırı perspektifleriyle tanınır.

Sohbet mesajları listesiyle de `chat` çağrısı yapabilirsiniz:
...
```
    ...
    **Şarkı Söyleyen Pusulanın Hikayesi**
     ...
     Crew'ım ve ben en garip hazineyi keşfettiğimizde sisli bir sabahtı - altın ya da mücevher değil, aklında olsun, deniz ilahileri (sea shanties) mırıldanan bir pusula! Evet, doğru duydun! Bu tuhaf küçük alet, işaret ettiği yöne göre farklı ezgiler mırıldanırdı.
    
    Kuzey, kayıp aşklar hakkında melankolik bir balad getirirken, Güney, en huysuz denizcimiz olan Tek Gözlü Pete'in bile tahta bacağına tempo tutturmasına neden olan neşeli bir melodi söylerdi. Ama işin ilginç yanı şuydu - Batı'yı gösterdiğinde, hiçbirimizin daha önce duymadığı, antik bir dilde sözleri olan gizemli bir melodi söylerdi.
    
    Maceracı tipler olduğumuz için (ve belki de biraz aptalca), o batı şarkısını üç gün üç gece takip ettik. Pusula bizi tehlikeli sulardan, seraplar gibi parıldayan adaların yanından geçirdi ve sonunda suyun sıvı zümrüt gibi parladığı gizli bir koya ulaştık.
    
    Ve orada dostum, en büyük hazineyi bulduk - zenginlik değil, büyülenmiş pusulalarının geri getirilmesini yüzyıllardır bekleyen bir deniz insanı ailesi! İyiliğimizi her türlü fırtınada güvenli geçiş ve üç gerçek hazine adasının gizli konumlarıyla ödüllendirdiler.
    
    *göz kırpar ve hayali bir şişeden bir yudum alır*
    
    Bazen en iyi maceralar en garip şarkıları takip etmekten gelir, anlıyor musun (savvy)?

## Akış (Streaming) Desteği

Her yöntem, `stream_` ön eki aracılığıyla akışı destekler.
...
```
    ...
    **Y Combinator Kurucu Ortağı**: 2005 yılında dünyanın en başarılı girişim hızlandırıcılarından biri olan Y Combinator'ı kurmuştur. Y Combinator; Airbnb, Dropbox, Stripe, Reddit ve yüzlerce diğer şirketi finanse etmiştir.
    ...
    **Viaweb**: 1990'larda, 1998'de Yahoo tarafından yaklaşık 49 milyon dolara satın alınan ve Yahoo Store olan ilk web tabanlı yazılım şirketlerinden biri olan Viaweb'i ortaklaşa kurmuştur.
    
    **Denemeler**: paulgraham.com adresindeki web sitesinde yayınladığı girişimler, programlama ve teknoloji üzerine birçok etkili deneme yazmıştır. Denemeleri teknoloji topluluğunda geniş çapta okunur ve bir girişimin nasıl kurulacağı, neyin iyi bir programcı yaptığı ve inovasyonun doğası gibi konuları kapsar.
    
    **Sanat ve Akademi**: Harvard'dan Bilgisayar Bilimleri doktorası vardır ve ayrıca Rhode Island Tasarım Okulu ile Floransa'daki Accademia di Belle Arti'de resim eğitimi almıştır.
    
    Graham, girişim ekosistemindeki en etkili isimlerden biri olarak kabul edilir ve girişimcilik ile teknoloji girişimleri hakkındaki modern düşüncenin şekillenmesine yardımcı olmuştur.
...
```
    ...
    **Yazarlık**: Graham, paulgraham.com adresindeki web sitesinde yayınladığı girişimler, teknoloji, programlama ve girişimcilik üzerine düşündürücü denemeleriyle tanınır. Denemeleri teknoloji çevrelerinde geniş çapta okunur ve bir girişimin nasıl kurulacağı, inovasyonun doğası ve teknolojik trendler gibi konuları kapsar.
    
    **Etki**: Hem Y Combinator'ın başarısı hem de birçok insanın girişimcilik ve teknoloji hakkında nasıl düşündüğünü şekillendiren yazılarıyla Silikon Vadisi'nin girişim ekosistemindeki en etkili kişilerden biri olarak kabul edilir.
    
    Teknik uzmanlık, iş zekası ve net yazım tarzının birleşimi, onu yirmi yılı aşkın süredir teknoloji endüstrisinde önde gelen bir ses haline getirmiştir.

## Asenkron Kullanım

Her senkron yöntemin bir asenkron karşılığı vardır.
...
```
    ...
    **Programlama**: Programlama topluluğunda saygın bir isimdir; özellikle Lisp programlama dili üzerindeki çalışmalarıyla tanınır. "On Lisp" ve "ANSI Common Lisp" gibi etkili kitaplar yazmıştır.
    
    **Denemeler**: Graham, paulgraham.com adresindeki web sitesinde yayınladığı girişimler, teknoloji, programlama ve toplum üzerine geniş çapta okunan denemeler yazar. "Do Things That Don't Scale" ve "How to Start a Startup" gibi denemeleri teknoloji dünyasında temel okuma parçaları kabul edilir.
    
    **Girişimci**: Y Combinator'dan önce, çevrimiçi mağaza oluşturmak için kullanılan ilk web tabanlı uygulamalardan biri olan Viaweb'i ortaklaşa kurmuştur; bu şirket 1998'de Yahoo tarafından yaklaşık 49 milyon dolara satın alınarak Yahoo Store olmuştur.
    
    **Sanat geçmişi**: İlginç bir şekilde, sanat alanında da bir geçmişi vardır ve resim okumuştur; bu durum teknolojiye bakış açısı, yaratıcılık ve estetik anlayışını etkilemektedir.
    
    Graham, Silikon Vadisi ve genel girişim ekosistemindeki en etkili seslerden biri olarak kabul edilir.
...
```
    ...
    assistant: Paul Graham bir bilgisayar programcısı, girişimci, risk sermayedar ve denemecidir. İşte tanındığı başlıca noktalar:
...
```
    ...
    Kaynak: https://news.microsoft.com/source/features/ai/6-ai-trends-you'll-see-more-of-in-2025/ - 2025'te yeni nesil YZ destekli aracılar daha fazlasını yapacak - hatta sizin adınıza belirli görevleri yürütecek.  
    ...
    Kaynak: https://news.microsoft.com/source/features/ai/6-ai-trends-you'll-see-more-of-in-2025/ - Microsoft'un iş ve sektör Copilot kurumsal başkan yardımcısı Charles Lamanna, "Aracıları YZ çağının uygulamaları olarak düşünün," diyor.
    ...
    Kaynak: https://news.microsoft.com/source/features/ai/6-ai-trends-you'll-see-more-of-in-2025/ - OpenAI o1 gibi gelişmiş muhakeme yeteneklerine sahip modeller, karmaşık problemleri insanların düşünme şekline benzer mantıksal adımlarla zaten çözebiliyor...
...
Anthropic kullanarak artık araç sonuçlarınızın belirli kısımlarına atıfta bulunmak için sunucu tarafı atıfları kullanabilirsiniz.

LLM bir araç sonucuna atıf yaparsa, atıf çıktıda kaynak, başlık ve atıfta bulunulan içeriği içeren bir `CitationBlock` olarak görünecektir.

Bunu uygulamada yapmanın birkaç yolunu ele alalım.

Önce, atıf yapılabilir bir blok döndüren sembolik (dummy) bir araç/fonksiyon tanımlayalım.
...
### Aracılar + Atıf Yapılabilir Araçlar (Agents + Citable Tools)

Çıktıda aynı atıfları almak için bu araçları doğrudan `FunctionAgent` gibi hazır aracılarda da kullanabilirsiniz.
...
agent = FunctionAgent(
    tools=[search_tool],
    llm=llm,
    # Statik sonuç döndüren sahte bir aracımız olduğu için LLM belirteçlerini boşa harcamak istemiyoruz
...
from llama_index.core.llms import CitationBlock
        ...
        print("Atıfta Bulunulan İçerik:\n", block.cited_content.text)
        print("----" * 20)
```

    Arama sonuçlarına dayanarak, LlamaIndex ve LLM'lerin birlikte nasıl çalıştığını açıklayabilirim:
    
    LLM'ler, bilgi üretimi ve muhakeme için olağanüstü bir teknoloji parçasıdır. Büyük miktarda halka açık veri üzerinde önceden eğitilmişlerdir. Ancak temel bir zorluk vardır: LLM'leri kendi özel verilerimizle en iyi nasıl güçlendirebiliriz?
    
    LlamaIndex, verilerinizi LLM'ler tarafından kullanılabilir hale getirmek için verilerinizi yüklemenize, dizinlemenize ve sorgulamanıza olanak tanıyan bir veri çerçevesidir.
    
    --------------------------------------------------------------------------------
    Kaynak:  https://docs.llamaindex.ai
    Başlık:  LLM'ler ve LlamaIndex hakkında gerçekler
    Atıfta Bulunulan İçerik:
     LLM'ler, bilgi üretimi ve muhakeme için olağanüstü bir teknoloji parçasıdır. Büyük miktarda halka açık veri üzerinde önceden eğitilmişlerdir. Ancak temel bir zorluk vardır: LLM'leri kendi özel verilerimizle en iyi nasıl güçlendirebiliriz?
    --------------------------------------------------------------------------------

### Manuel Araç Çağırma + Atıflar (Manual Tool Calling + Citations)

Atıf yapılabilir bir blok döndüren aracımızı kullanarak, manuel bir aracı döngüsünde LLM'i verilen araçla manuel olarak çağırabiliriz.

LLM araç çağırmayı bıraktığında, nihai yanıtı döndürebilir ve yanıttaki atıfları ayrıştırabiliriz.
...
```
    ...
    LlamaIndex, LLM uygulamaları oluşturmanıza yardımcı olan bir "veri çerçevesidir". Entegrasyon, temel bir zorluğu ele alarak çalışır: LLM'ler bilgi üretimi ve muhakeme için olağanüstü bir teknoloji parçası olsa da ve büyük miktarda halka açık veri üzerinde önceden eğitilmiş olsalar da, kendi özel verilerimizle LLM'ler için veri güçlendirme işlemini gerçekleştirmeye yardımcı olacak kapsamlı bir araç setine ihtiyacımız vardır.
    
    İşte LlamaIndex ve LLM'lerin birlikte nasıl çalıştığı:
    
    ## Veri Entegrasyonu
    
    LlamaIndex, mevcut veri kaynaklarınızı ve veri formatlarınızı (API'ler, PDF'ler, belgeler, SQL vb.) yüklemek için veri bağlayıcıları sunar, böylece özel verilerinizi LLM'lerin çalışabileceği bir formata getirmenize olanak tanır.
    
    ## Veri Yapılandırma
    
    LlamaIndex, verilerinizin LLM'ler ile kolayca kullanılabilmesi için verilerinizi yapılandırma yolları (dizinler, grafikler) sağlar. Bu yapılandırma, verilerinizin LLM tarafından erişilebilir ve taranabilir olması için çok önemlidir.
    
    ## Gelişmiş Sorgulama
    
    LlamaIndex, verileriniz üzerinde gelişmiş bir erişim/sorgulama arayüzü sağlar: Herhangi bir LLM giriş istemini gönderin, karşılığında getirilen bağlamı ve bilgiyle güçlendirilmiş çıktıyı alın. Bu, LLM'e bir soru sorduğunuzda LlamaIndex'in verilerinizden ilgili bilgileri getirdiği ve LLM'in yanıtını iyileştirmek için bağlam olarak sağladığı anlamına gelir.
    
    ## Uygulama Entegrasyonu
    
    LlamaIndex, dış uygulama çerçevenizle (örneğin LangChain, Flask, Docker, ChatGPT, başka herhangi bir şey ile) kolay entegrasyonlar sağlar, mevcut sistemlere dahil edilmesini esnek hale getirir.
    
    Çerçeve, farklı seviyelerdeki kullanıcılar için erişilebilir olacak şekilde tasarlanmıştır: LlamaIndex'in üst düzey API'si, yeni başlayan kullanıcıların 5 satır kodla verilerini yüklemelerine ve sorgulamalarına olanak tanırken, alt düzey API'ler ileri düzey kullanıcıların her türlü modülü (veri bağlayıcıları, dizinler, erişimciler, sorgu motorları, yeniden sıralama modülleri) ihtiyaçlarına göre özelleştirmelerine ve genişletmelerine olanak tanır.
    ...
    Bağlam
    LLM'ler, bilgi üretimi ve muhakeme için olağanüstü bir teknoloji parçasıdır.
    ...
    LLM'ler için bu veri güçlendirme işlemini gerçekleştirmeye yardımcı olacak kapsamlı bir araç setine ihtiyacımız var.
    
    Önerilen Çözüm
    LlamaIndex işte burada devreye giriyor. LlamaIndex, LLM uygulamaları oluşturmanıza
    yardımcı olan bir "veri çerçevesidir". Aşağıdaki araçları sağlar:
    
    Mevcut veri kaynaklarınızı ve veri formatlarınızı (API'ler, PDF'ler, belgeler, SQL vb.)
    yüklemek için veri bağlayıcıları sunar.
    Verilerinizin LLM'ler ile kolayca kullanılabilmesi için verilerinizi yapılandırma
    yolları (dizinler, grafikler) sağlar.
    Verileriniz üzerinde gelişmiş bir erişim/sorgulama arayüzü sağlar:
    Herhangi bir LLM giriş istemini gönderin, getirilen bağlamı ve bilgiyle güçlendirilmiş çıktıyı alın.
    Dış uygulama çerçevenizle (örneğin LangChain, Flask, Docker, ChatGPT, diğerleri ile)
    kolay entegrasyonlar sağlar.
    LlamaIndex hem yeni başlayan hem de ileri düzey kullanıcılar için araçlar sağlar.
    Üst düzey API'miz, yeni başlayan kullanıcıların LlamaIndex'i 5 satır kodla verilerini
    yüklemek ve sorgulamak için kullanmalarına olanak tanır. Alt düzey API'lerimiz,
    ileri düzey kullanıcıların ihtiyaçlarına göre herhangi bir modülü (veri bağlayıcıları,
    dizinler, erişimciler, sorgu motorları, yeniden sıralama modülleri) özelleştirmelerine
    ve genişletmelerine olanak tanır.
